		...
		if (deathEvents[subEvent]) then
			...
			if (subEvent == 'SPELL_CAST_SUCCESS') then -- used for tracking notarget spell casts
				...
				if (noTargetFake[arg1] and ((filterByBlacklist and not blacklist[spellID]) or (not filterByBlacklist and whitelist[spellID]))) then
					...
					if ( duration <= durationMin or duration >= durationMax) then return end -- abort if duration is restricted -- commented by Eskiso
					...
					if (unit.auras[arg1]) then -- aura already exists, update it
						...
						local texture, _ = C_Spell.GetSpellTexture(spellID)
						unit:AddAura(Aura:New(currentTime, unit, arg1, name, texture, duration, currentTime + duration, 0))
					end
...
function Ellipsis:PLAYER_TOTEM_UPDATE(slot)
	...
	if (spellID) then -- proper totem
		totemTexture, _ = C_Spell.GetSpellTexture(spellID)
	else
		if (not summonSpellID) then return end -- not even enough info to fake it

		totemTexture, _ = C_Spell.GetSpellTexture(summonSpellID)
...
function Ellipsis:UNIT_AURA(unitTag)
	...
	local _spellName, _, stackCount, _, duration, expireTime, unitCaster, _, _, spellID = UnitAura(unitTag, 1, filter)
	local guid	= UnitGUID(unitTag)
	local unit	= activeUnits[guid]

	if (not _spellName and not unit) then return end -- none of our auras here, and not already tracking this unit, bailout
	...
	while (_spellName) do -- only scanning our auras on target
		spellName = _spellName.name
		stackCount = (_spellName.charges or 1)
		duration = _spellName.duration
		expireTime = _spellName.expirationTime
		unitCaster = _spellName.sourceUnit
		spellID = _spellName.spellId
			...
			if (((filterByBlacklist and not blacklist[spellID]) or (not filterByBlacklist and whitelist[spellID])) and
				...
				if (noTargetRedirect[spellID]) then -- spell needs to be redirected to notarget unit
					...
					if (not aura) then -- no aura exists yet, create one
						local texture, _ = C_Spell.GetSpellTexture(spellID)
						--local texture = _spellName.icon
						aura = noTarget:AddAura(Aura:New(currentTime, noTarget, spellID, spellName, texture, duration, expireTime, stackCount))
					...
					if (not aura) then -- no aura exists yet, create one
						...
						local texture, _ = C_Spell.GetSpellTexture(spellID)
						aura = unit:AddAura(Aura:New(currentTime, unit, spellID, spellName, texture, duration, expireTime, stackCount))
		...
		index = index + 1
		_spellName, _, stackCount, _, duration, expireTime, unitCaster, _, _, spellID = UnitAura(unitTag, index, filter)
	end
